...
\begin{tcolorbox}[
    ...
    \[
    x_{k+1} = \overline A_k x_k + \overline B_k u_k
    \]
\end{tcolorbox}
...
\node (Jquad) at (0,0)
{$\mathcal{J}_k(x) = x' \overline K_k \,x$};

\node[gray] (quadtext) at (-4,0.6)
{$\mathcal{J}_k(x)$ is a quadratic form of $x$};

\draw[->, gray] (quadtext) -- (Jquad);

\node[red] (unknown) at (4,0.6)
{\emph{Unknown at this point!}};

\draw[->, red] (unknown) -- (Jquad);

\node[scale=3] at (0,-1) {$\Downarrow$};

\node (Jnext) at (0,-2)
{$
\mathcal{J}_{k+1}(\overline A_k x + \overline B_k u)
=
(\overline A_k x + \overline B_k u)' \,\overline K_{k+1}\,(\overline A_k x + \overline B_k u)
$};

\node[gray] (plug) at (4,-3.1)
{Plug into \textbf{Bellman equation}};

\node at (0,-3.1) {$\Downarrow$};
...
% +
% u'
% \underbrace{(R_k + \overline B_k' \overline K_{k+1} \overline B_k)}_{\hat R_k}
% u \\
% &\;
% +
% 2x'
% \underbrace{(\overline P_k + \overline A_k'\, \overline K_{k+1}\, \overline B_k)}_{\hat S_k}
% u
% \end{aligned}
% $};
{$
...
\begin{frame}{NOTES}
        \begin{questionbox}
            can't move the whole thing to the left
        \end{questionbox}

        \begin{questionbox}
            a frame for the last eq? or copy it to the next slide
        \end{questionbox}
\end{frame}
...
\overline R_k\,u + \overline S_k' x = 0
\]
...
u_k^\ast
=
-\,\overline R_k^{-1}\,\overline S_k' \, x_k
\]
