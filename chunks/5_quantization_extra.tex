\commentslide{
	From now on, these slides are our thoughts on important quantization slides.
	It must be checked with Sree's slides!
}
...
\questionslide{
	\begin{itemize}
		...
		\item $\Omega$ undefined is fine? Should the integration limits be $\int\limits_0^T$ or $\int\limits_{0^m}^{T^m}$ instead?
		\item in the paper, they integrate over $\Omega$ and w.r.t. $dx$. Should it be wrt $t$ and between $(0,T)$ instead?
	\end{itemize}
}
