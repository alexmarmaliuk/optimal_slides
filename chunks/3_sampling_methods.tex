...
\begin{frame}[t]{Comparing Sampling Methods: Sampling Density}
	...
	\begin{block}{Sampling Density}
		Given a sampling sequence $t_0=0,\,t_1,\, \ldots, t_N =T$, we define the \emph{sampling density} $\sigma_{N,m}:[0,T] \to \mathbb{R}^{+}$ of any sampling method $m$ as
		\[
...
\begin{frame}{Comparing Sampling Methods: Normalized Cost}
	...
	\begin{block}{Normalized Cost}
		Given an interval length $T$, and a number of samples $N$, the \emph{normalized cost} $c_{N,m}$ of any sampling method $m$ is defined as
		\[
	...
	\note<1>{\begin{itemize}
			...
			\item what is the gain by taking a sample and what is the cost by sampling it
		...
		\begin{columns}
			\begin{column}{0.7\textwidth}

				{
					\color{gray!80}
					\textit{
						$\ldots$ informally, the optimal controller of the discrete-time can be expressed as the continuous-time solution $K_\infty$ plus a correction term that is proportional to the square of the sampling period $\tau$.
					}
				}
			\end{column}
		\end{columns}
	...
	\uncover<1-2>{
		\begin{itemize}
			...
			\item Sample whenever the optimal $u$ changes by a fixed threshold $\Delta$, so after any sampling instant $t_k$, the next $t_{k+1}$ is determined s.t.
			      $$
				      \left\| \,u\,(t_{k+1}) - u(t_k)\, \right\|=\,\Delta
			      $$
		...
		\begin{example}[$\Delta=7$]
			...
			\begin{tikzpicture}[scale=0.98]
				...
				\begin{axis}[
						...
						extra x tick labels={\only<2->{$t_0$},\only<3->{$t_1$},\only<4->{$t_2$},\only<5->{$t_3$},\only<6->{$t_4$}},
