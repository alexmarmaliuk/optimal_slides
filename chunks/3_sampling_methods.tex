...
\questionslide{
	\begin{itemize}
		\item Is it okay that we started at $N=10$ and also the way we denoted it in the graph, or is the x axis too chaotic?
		\item Plot good? do $x$ labels $10$ and $60$ look weird?
	\end{itemize}
}
...
\begin{frame}{Periodic Sampling: Asymptotic Normalized Cost}
	...
	\begin{eqnarray*}
		...
		\pause
		& = & \lim\limits_{N \to \infty} \frac{N^2}{T^2} \, \frac{x_0' \,\frac{XT^2}{2N^2} \, x_0 + x_0' \,o\left(N^{-2}\right)\,x_0}{x_{0}' \, K_\infty \,x_0}
		\pause
...
\questionslide{To make the elements appear step by step in slide 22, I did the tikzpicture directly in the frame instead of including the plot as pdf. Slide 21 is still the pdf version. I noted that they actually look different, which one is the prefered one, as so far we always include pdfs. Should we change the other plots too, to have the tikzpicture directly in the frame?}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}{Deterministic Lebesgue Sampling: Deriving the Relation for $N$}
	...
	\begin{eqnarray*}
		...
		\pause & = & \frac{1}{N} \cdot \underbrace{\int_{0}^T \, \left|\, \dot{u}\,(s) \,\right| \, ds}_{N\cdot\Delta}
	\end{eqnarray*}
...
\questionslide{\begin{itemize}
		\item For the last two reminder boxes with the equation $\left\| \,u\,(t_{t+1}) - u(t_k)\, \right\|=\,\Delta$, should we write it with the absolute value instead, so $| \,u\,(t_{t+1}) - u(t_k)\, | =\,\Delta$, as both of them are cases for scalar input?
		\item Wanted one more transition in between for slide 22, where the last equation appears without the underbrace first and then with it. Didnt manage to do so, with only the equations changed their places and uncover didnt work :(
	\end{itemize}}
...
\questionslide{
	should we use \textbackslash displaystile for the integrals in fractions?
	If we do, integral sign looks better, but the expression explodes a bit.
